...
public final class SignFixtureFaceData {
    ...
    private final int doubleSidedMode;
    private final boolean bright;

    public static final int ROTATION_COUNT = 8;

    public static final int DOUBLE_SIDED_MODE_COUNT = 3;
    public static final int DOUBLE_SIDED_OFF = 0;
    public static final int DOUBLE_SIDED_ON = 1;
    public static final int DOUBLE_SIDED_ON_MIRRORED = 2;

    public SignFixtureFaceData(ItemStack item, SignStance stance, int rotation, float scale, float offset,
            int doubleSidedMode, boolean bright) {
        this.item = item;
        this.stance = stance;
    ...
    public static SignFixtureFaceData initial(ItemStack item) {
        return new SignFixtureFaceData(item, SignStance.FLAT, 0, 1.0f, 0f, DOUBLE_SIDED_OFF, false);
    }
    ...
    public SignFixtureFaceData withStance(SignStance newStance) {
        return new SignFixtureFaceData(item, newStance, rotation, scale, offset, doubleSidedMode, bright);
    }

    public SignFixtureFaceData withRotation(int newRotation) {
        return new SignFixtureFaceData(item, stance, newRotation, scale, offset, doubleSidedMode, bright);
    }

    public SignFixtureFaceData withScale(float newScale) {
        return new SignFixtureFaceData(item, stance, rotation, newScale, offset, doubleSidedMode, bright);
    }

    public SignFixtureFaceData withOffset(float newOffset) {
        return new SignFixtureFaceData(item, stance, rotation, scale, newOffset, doubleSidedMode, bright);
    }

    public SignFixtureFaceData withDoubleSided(int newDoubleSidedMode) {
        return new SignFixtureFaceData(item, stance, rotation, scale, offset, newDoubleSidedMode, bright);
    }

    public SignFixtureFaceData withBright(boolean newBright) {
        return new SignFixtureFaceData(item, stance, rotation, scale, offset, doubleSidedMode, newBright);
    }
    ...
    public CompoundTag save(HolderLookup.Provider registries) {
        ...
        tag.putInt(NBT_DOUBLE_SIDED_MODE, doubleSidedMode);
    ...
    public static SignFixtureFaceData load(CompoundTag tag, HolderLookup.Provider registries) {
        ...
        int doubleSidedMode = tag.getInt(NBT_DOUBLE_SIDED_MODE);
        boolean bright = tag.getBoolean(NBT_BRIGHT);
        return new SignFixtureFaceData(item, stance, rotation, scale, offset, doubleSidedMode, bright);
    }
}
